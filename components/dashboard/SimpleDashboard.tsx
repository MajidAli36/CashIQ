...
export function SimpleDashboard() {
  ...
  const stats = useMemo(() => ({
    totalBalance: enabledWallets.reduce((s, w) => s + getWalletBalance(w.id, bid), 0),
    monthIncome:  getTotalIncome(monthStart, today, bid),
    monthExpense: getTotalExpense(monthStart, today, bid),
    todayIncome:  getTotalIncome(today, today, bid),
    todayExpense: getTotalExpense(today, today, bid),
  }), [enabledWallets, bid, today, monthStart, getWalletBalance, getTotalIncome, getTotalExpense])

  const toReceive = getTotalToReceive(bid)
  const toGive    = getTotalToGive(bid)

  const recent = getRecent(20, bid)
  ...
  return (
    <div className="pb-8" style={{ background: '#F8FAFC' }}>
      ...
      <div className="bg-navy-gradient px-5 pt-10 pb-8 relative overflow-hidden">
        ...
        {/* Mode badge + shop name */}
        <div className="flex items-center justify-between mb-2">
          {shop.name ? (
            <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#00C4B4' }}>
              {shop.name}
            </p>
          ) : <span />}
          <button
        ...
        <p className="text-white font-black text-4xl tracking-tight mb-5">
      ...
      <div className="px-4 mt-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: '#FFEBEB' }}>
            ...
            <div className="flex-1 min-w-0">
              {toReceive > 0 && (
                <p className="text-sm font-semibold" style={{ color: '#92400E' }}>
                  You have {formatCurrency(toReceive)} to collect
                </p>
              )}
              {toGive > 0 && (
